      ...
      <aside className="w-full md:w-1/3 flex flex-col gap-6">
        ...
        <div className="bg-surface-container-lowest rounded-3xl p-8 shadow-[0_10px_30px_rgba(11,28,48,0.06)] flex flex-col items-center text-center">
          ...
          <div className="mt-8 w-full space-y-4">
            <Button
              variant="unstyled"
              className="w-full py-4 bg-primary-container text-on-primary-container rounded-full font-bold shadow-lg shadow-primary-container/20 flex items-center justify-center gap-2 hover:brightness-105 active:scale-95 transition-all"
            >
              <span
            ...
            <Button
              variant="unstyled"
              className="w-full py-3 bg-surface-container-low text-primary rounded-full font-semibold hover:bg-surface-container-high transition-colors"
            >
              View Public Profile
            </Button>
          </div>
        ...
        <div className="bg-surface-container-lowest rounded-3xl p-8 shadow-[0_10px_30px_rgba(11,28,48,0.06)]">
          <h2 className="text-xs font-bold uppercase tracking-[0.05em] text-outline mb-6">
            Personal Info
          </h2>
          ...
          <div className="relative space-y-10 pl-6 border-l-2 border-outline-variant/20">
            {experiences.map((exp) => (
              <div key={exp.id} className="relative">
                <div
                  className={`absolute -left-[31px] top-0 w-4 h-4 rounded-full ring-4 ring-surface-container-lowest shadow-sm ${exp.isCurrent ? "bg-primary" : "bg-outline-variant"}`}
                ></div>
                <div className="flex flex-col gap-2">
                  <span
                    className={`text-xs font-bold uppercase tracking-widest ${exp.isCurrent ? "text-primary" : "text-outline"}`}
                  >
                    {exp.period}
                  </span>
        ...
        <div className="bg-surface-container-lowest rounded-3xl p-8 shadow-[0_10px_30px_rgba(11,28,48,0.06)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {resumes.map((resume) => (
              <div
                key={resume.id}
                className="p-6 bg-surface-container-low rounded-2xl flex items-center justify-between hover:bg-surface-container-high transition-colors cursor-pointer group"
              >
                <div className="flex items-center gap-4 overflow-hidden">
                  <div className="w-12 h-12 bg-[#ffffff] rounded-xl flex items-center justify-center shadow-sm shrink-0">
                    <span className={`material-symbols-outlined text-3xl ${resume.iconColorClass}`}>
                      description
                    </span>
                  </div>
                ...
                <span
                  className={`material-symbols-outlined text-outline transition-colors ${resume.hoverColorClass}`}
                >
                  download
                </span>
              </div>
