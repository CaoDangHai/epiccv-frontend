      ...
      <div className="p-8 pb-24 md:pb-8 flex-1 overflow-y-auto">
        <div className="max-w-6xl mx-auto animate-in fade-in duration-300">
          <div className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <h2 className="text-4xl font-bold tracking-tight text-on-background mb-2">
                Analysis History
              </h2>
              <p className="text-slate-500 text-lg">
                Review and manage your past AI resume optimizations.
              </p>
            </div>
            <div className="relative group">
              <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[var(--color-primary)] transition-colors">
                search
              </span>
              <Input
                value={searchQuery}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {summaryStats.map((stat) => (
              <div
                key={stat.id}
                className="bg-surface-container-lowest p-6 rounded-2xl shadow-[0_10px_30px_rgba(11,28,48,0.06)] flex items-center gap-4 border border-outline-variant/5"
              >
                <div
                  className={`w-12 h-12 rounded-xl ${stat.iconBgClass} flex items-center justify-center ${stat.iconColorClass}`}
                >
          ...
          <div className="bg-surface-container-lowest rounded-2xl shadow-[0_10px_30px_rgba(11,28,48,0.06)] overflow-hidden border border-outline-variant/5">
                  ...
                  {records.length > 0 ? (
                    records.map((record) => (
                      <tr
                        ...
                        onClick={() => (window.location.href = `#/reports/${record.id}`)}
                      >
                        ...
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded-md bg-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-400">
                        ...
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-3">
                            <div className="flex-1 h-1.5 w-24 bg-surface-container rounded-full overflow-hidden">
                              <div
                                className={`${record.matchBgClass} h-full rounded-full transition-all duration-1000`}
                                style={{ width: `${record.matchScore}%` }}
                              ></div>
                            </div>
                            <span className={`text-sm font-bold ${record.matchTextClass}`}>
                              {record.matchScore}%
                            </span>
                          </div>
                        </td>
                        <td className="px-8 py-6 text-right" onClick={(e) => e.stopPropagation()}>
                          <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <Button
                              variant="unstyled"
                              className="p-2 text-error hover:bg-error-container/50 rounded-lg transition-all"
                              title="Delete"
                            >
            ...
            <div className="px-8 py-6 flex flex-col sm:flex-row items-center justify-between bg-surface-container-low/30 border-t border-surface-container gap-4">
              <p className="text-sm text-slate-500">
                ...
                to <span className="font-semibold text-on-background">{records.length}</span> of{" "}
                <span className="font-semibold text-on-background">{totalResults}</span> results
              </p>
              <div className="flex items-center gap-2">
                <Button
                  variant="unstyled"
                ...
                >
                  <span className="material-symbols-outlined text-sm mr-1 group-hover:-translate-x-0.5 transition-transform">
                    chevron_left
                  </span>
