...
export const useAnalysis = () => {
  ...
  }, [handleSelectReport, location.state?.targetReportId, navigate]);

  const onTabChange = (tab: string) => {
    if ((tab === "phases" || tab === "documentation" || tab === "status") && !roadmap) {
      toast("Please generate a roadmap to unlock this section.", { icon: "lock" });
      return;
    }
    setActiveTab(tab);
  };
  ...
  const handleCloseDrawer = () => {
    ...
    window.setTimeout(() => setSelectedPhase(null), 300);
  };
