          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <span
                  className={`text-[10px] font-black tracking-widest uppercase ${active ? "text-[var(--color-primary)]" : "text-slate-400"}`}
                >
                  Phase 0{phase.id}
                </span>
                {!unlocked && (
              ...
              <span
                className={`px-3 py-1 rounded-full text-[10px] font-bold ${unlocked ? (done ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-800") : "bg-slate-200 text-slate-500"}`}
              >
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex -space-x-2">
                <img
                  ...
                  src={`https://ui-avatars.com/api/?name=User+${phase.id}&background=random`}
                  alt="User"
                />
                <img
                  className="w-6 h-6 rounded-full border-2 border-white bg-slate-200"
                  src={`https://ui-avatars.com/api/?name=Dev+${phase.id}&background=random`}
                  alt="Dev"
                />
      ...
      <div className="bg-[var(--color-primary)] rounded-2xl p-8 text-white relative overflow-hidden shadow-lg shadow-blue-500/20">
        <div className="relative z-10">
          ...
          <p className="text-white/80 text-sm mb-6 leading-relaxed">
            {activePhase === Object.keys(phases).length
              ? "You've reached the final phase! Review your reports for interview prep."
              : `Progress is looking great. Move to Phase 0${activePhase + 1} once you're ready.`}
          </p>
          {activePhase < Object.keys(phases).length && isPhaseUnlocked(activePhase + 1) ? (
            ...
            <Button
              variant="unstyled"
              className="bg-white/20 backdrop-blur-sm text-white px-6 py-2 rounded-full font-bold text-xs cursor-default"
            >
              {activePhase === Object.keys(phases).length
                ? "Roadmap Complete"
